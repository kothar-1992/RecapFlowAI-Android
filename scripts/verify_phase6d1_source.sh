...
require_marker 'rootProject.name = "RecapFlowAI_Phase6D1"' "settings.gradle.kts"
require_marker 'versionName = "1.0-phase6d1"' "app/build.gradle.kts"
require_marker 'android:id="@+id/reviewAudioTabButton"' "$layout_file"
require_marker 'android:id="@+id/audioEnabledSwitch"' "$layout_file"
require_marker 'android:id="@+id/audioPolicyGroup"' "$layout_file"
require_marker 'private fun onUserChangedAudio()' "$main_file"
require_marker 'previewPlayer.volume = if (isSourcePreview)' "$main_file"
require_marker 'object AudioCompiler' "$compiler_file"
require_marker '.setRemoveAudio(removeAudio)' "$render_file"
require_marker 'freezeFrame != null && mediaInfo.hasAudio && !removeAudio' "$render_file"
        ...
        if state == "code":
            ...
            if char == '"':
                ...
                if not stack or stack.pop() != pairs[char]:
                    raise SystemExit(f"UNBALANCED KOTLIN DELIMITER in {source} near offset {index}")
...
echo "PASS: RecapFlowAI Phase 6D.1 Audio markers, XML/resources, shell syntax, and secret scan are valid."
