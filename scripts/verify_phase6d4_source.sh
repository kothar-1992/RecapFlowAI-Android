...
require_marker 'rootProject.name = "RecapFlowAI_Phase6D4"' "settings.gradle.kts"
require_marker 'versionName = "1.0-phase6d4"' "app/build.gradle.kts"
require_marker 'android:id="@+id/audioMixButton"' "$layout_file"
require_marker 'android:id="@+id/mixSourceVolumeSlider"' "$layout_file"
require_marker 'android:id="@+id/mixAddedVolumeSlider"' "$layout_file"
require_marker 'AudioPolicy.MIX -> R.id.audioMixButton' "$main_file"
require_marker 'mixSourceVolume = AudioCompiler.DEFAULT_MIX_SOURCE_LINEAR_GAIN' "$main_file"
require_marker 'mixAddedVolume = AudioCompiler.DEFAULT_MIX_LINEAR_GAIN' "$main_file"
require_marker 'settings.policy == AudioPolicy.MIX' "$compiler_file"
require_marker 'mixesSourceAudio = true' "$compiler_file"
require_marker 'AudioPolicy.MIX,' "$validator_file"
require_marker 'class StereoPcmMixAudioProcessor' \
  "app/src/main/kotlin/com/recapflow/ai/media/render/StereoPcmMixAudioProcessor.kt"
require_marker 'StereoPcmMixAudioProcessor(audio.linearGain)' "$render_file"
require_marker 'normalizeForMix = compiledAudio.mixesSourceAudio' "$render_file"
require_marker '.setIsLooping(true)' "$render_file"
require_marker 'mixKeepsSourceAndCarriesIndependentTrackGains' \
  "app/src/test/kotlin/com/recapflow/ai/media/edit/AudioCompilerTest.kt"
require_marker 'mixWithValidAssetAndIndependentGainsIsAccepted' \
  "app/src/test/kotlin/com/recapflow/ai/media/edit/EditPlanValidatorTest.kt"
require_marker 'class StereoPcmMixAudioProcessorTest' \
  "app/src/test/kotlin/com/recapflow/ai/media/render/StereoPcmMixAudioProcessorTest.kt"
...
echo "PASS: RecapFlowAI Phase 6D.4 Mix Audio markers and static source checks are valid."
